...
|---------------|---------------|---------|
| `fix`, `bug`, `hotfix` | `fix/` | `fix/login-bug` |
| anything else | `feat/` | `feat/new-model-support` |
...
```bash
PROJECT_ROOT=$(pwd)                    # e.g., /Users/dev/projects/rag-simple
PROJECT_NAME=$(basename "$PROJECT_ROOT") # e.g., rag-simple
PARENT_DIR=$(dirname "$PROJECT_ROOT")    # e.g., /Users/dev/projects
WORKTREE_DIR="${PARENT_DIR}/${PROJECT_NAME}-{name}"  # e.g., /Users/dev/projects/rag-simple-auth
```

**Examples:**
- `/worktree auth` → Folder: `../rag-simple-auth/`, Branch: `feat/auth`
- `/worktree fix-embeddings` → Folder: `../rag-simple-fix-embeddings/`, Branch: `fix/fix-embeddings`
- `/worktree list` → Shows all worktrees
- `/worktree remove auth` → Removes `../rag-simple-auth/` worktree
...
**This is a monorepo. Copy .env files for the web app:**
...
```bash
# Navigate to new worktree and install dependencies (root package.json)
cd "${WORKTREE_DIR}"
...
3. Start dev server: PORT=3001 npm run dev
...
|-----------|--------|--------|
| /path/to/rag-simple | main | (main worktree) |
| /path/to/rag-simple-auth | feat/auth | active |
| /path/to/rag-simple-bugfix | fix/bugfix | active |
```
...
🔐 Environment: `.env.local` files copied

**Monorepo Structure:**
- `apps/web/` - Next.js frontend with RAG
...
|---|-----------|--------|---------|
| 1 | .../rag-simple | main | Main development |
| 2 | .../rag-simple-auth | feat/auth | Feature: Authentication |
| 3 | .../rag-simple-bugfix | fix/bugfix | Bugfix |
...
8. Provide next steps with IDE and dev server instructions
...
### Key Details
- This is a monorepo with Next.js web app
