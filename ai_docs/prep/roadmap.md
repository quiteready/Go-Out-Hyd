...
- [ ] Remove RAG/AI packages from `apps/web/package.json`: `@ai-sdk/google`, `@ai-sdk/react`, `ai`, `@google-cloud/aiplatform`, `@google-cloud/storage`
- [ ] Remove content rendering packages: `react-markdown`, `remark-gfm`
- [ ] Remove payment package: `stripe`
- [ ] Remove any other unused RAG-related dependencies identified in `package.json`
- [ ] Run `npm install` to clean the lockfile
- [ ] Remove RAG deployment scripts from root `package.json`: `deploy:processor:*`, `deploy:gcs-handler:*`, `deploy:task-processor:*`, `setup:gcp:*`, `stripe:listen`, `storage:setup`
...
- [ ] Delete `apps/rag-processor/` directory entirely (Python RAG processing service)
- [ ] Audit repo root for any other non-web directories: Cloud Run configs, GCS handler dirs (`apps/rag-gcs-handler/`, `apps/rag-task-processor/`), Python scripts, GCP deployment files -- delete all of them
- [ ] Remove any workspace references to deleted directories from root `package.json` workspaces config

**Routes to delete:**

- [ ] Delete `app/(auth)/` directory (all auth routes: login, sign-up, forgot-password, update-password, confirm, sign-up-success, error)
- [ ] Delete `app/(protected)/` directory (all protected routes: chat, documents, history, profile, admin)
- [ ] Delete `app/api/chat/` directory
- [ ] Delete `app/api/documents/` directory
- [ ] Delete `app/api/webhooks/` directory (if exists)

**Components to delete:**

- [ ] Delete `components/chat/` directory
- [ ] Delete `components/documents/` directory
- [ ] Delete `components/history/` directory
- [ ] Delete `components/auth/` directory
- [ ] Delete `components/profile/` directory
- [ ] Delete `components/landing/` directory (RAG-specific landing sections: HeroSection, FeaturesSection, RAGDemoSection, PricingSection, FAQSection, etc.)
- [ ] Delete `components/layout/AppSidebar.tsx` (dashboard sidebar, not needed for public site)
- [ ] Delete `components/NavbarThemeSwitcher.tsx` and `components/SidebarThemeSwitcher.tsx` (no dark mode)
- [ ] Delete `components/Logo.tsx` (rebuild with GoOut Hyd wordmark)

**Lib files to delete:**

- [ ] Delete `lib/embeddings/` directory
- [ ] Delete `lib/search/` directory
- [ ] Delete `lib/rag/` directory
- [ ] Delete `lib/documents.ts`, `lib/storage.ts`
- [ ] Delete `lib/attachments.ts`, `lib/attachments-client.ts`
- [ ] Delete `lib/chat-utils.ts`, `lib/chat-utils-client.ts`
- [ ] Delete `lib/google-cloud.ts`
- [ ] Delete all `lib/upload-*.ts` files (upload queue, error handling, file validation)
- [ ] Delete all `lib/processing-*.ts` files (processing constants/utils)
- [ ] Delete `lib/history.ts`, `lib/persistence.ts`

**Other files to delete:**

- [ ] Delete `contexts/ChatStateContext.tsx` (if exists)
- [ ] Delete `app/(public)/cookies/` directory (not in GoOut Hyd plan)
...
- [ ] Update `lib/env.ts`: remove `GEMINI_API_KEY`, `GOOGLE_CLOUD_PROJECT_ID`, `GOOGLE_CLOUD_REGION`, `GOOGLE_CLOUD_SERVICE_ACCOUNT_KEY`, `GOOGLE_CLOUD_STORAGE_BUCKET`
- [ ] Add `RESEND_API_KEY` to `lib/env.ts` server schema with Zod string validation (mark as `.optional()` so Phases 1-3 boot without it; Phase 4 will use it)
- [ ] Keep: `DATABASE_URL`, `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `NEXT_PUBLIC_APP_URL`
- [ ] Replace `middleware.ts` body with a passthrough: remove all Supabase session checks and auth redirect logic, return `NextResponse.next()` unconditionally. No DB call on every request when there's no auth to enforce. Add a comment: `// Phase 2: Re-enable Supabase session checks and auth redirects here when authentication is added`
- [ ] Update `next.config.ts`: add Supabase Storage domain to `images.remotePatterns` for Next.js Image component (e.g., `{ protocol: 'https', hostname: '*.supabase.co' }`)
- [ ] Update root and web `package.json` name and description from rag-simple references to GoOut Hyd
- [ ] Run `npm run dev` to confirm the app boots cleanly after all deletions
...
- [ ] Update `tailwind.config.ts` with GoOut Hyd color tokens:
...
- [ ] Add font family config to `tailwind.config.ts`: `heading: ['"DM Serif Display"', 'Georgia', 'serif']`, `body: ['"DM Sans"', 'system-ui', 'sans-serif']`
- [ ] Update `app/layout.tsx`: import DM Serif Display (weight 400) and DM Sans (weights 300, 400, 500) via `next/font/google`, apply to `<body>` className
- [ ] Update `app/globals.css` with GoOut Hyd CSS custom properties mapped to the color tokens
- [ ] Update root layout metadata: title "GoOut Hyd", description "Discover Hyderabad's best independent cafes and events"

### 1A.5 Database Schema

[Goal: Replace all RAG tables with GoOut Hyd's 5-table schema for cafes, images, menus, events, and leads]

- [ ] Delete all existing schema files in `lib/drizzle/schema/`: `documents.ts`, `document_chunks.ts`, `document_processing_jobs.ts`, `conversations.ts`, `messages.ts`, `users.ts`
- [ ] Create `lib/drizzle/schema/cafes.ts`:
...
- [ ] Create `lib/drizzle/schema/cafe-images.ts`:
...
- [ ] Create `lib/drizzle/schema/menu-items.ts`:
...
- [ ] Create `lib/drizzle/schema/events.ts`:
...
- [ ] Create `lib/drizzle/schema/cafe-leads.ts`:
...
- [ ] Update `lib/drizzle/schema/index.ts` to export all 5 new table schemas and 4 enums
- [ ] Run `npm run db:generate` to create migration
- [ ] Run `npm run db:migrate` to apply migration to Supabase
...
- [ ] Create public bucket in Supabase Storage via dashboard (bucket name: `images`)
- [ ] Configure bucket public access policy (allow public reads, no RLS required for public bucket)
- [ ] Upload a set of placeholder images for development: 4 cafe cover photos, 12-16 gallery photos (3-4 per cafe), 6 event cover images
- [ ] Note the public URL pattern (e.g., `https://<project-ref>.supabase.co/storage/v1/object/public/images/...`) for use in seed data
...
- [ ] Create seed script at `lib/drizzle/seed.ts` (or update existing):
...
- [ ] **Do NOT create `lib/queries/areas.ts`** -- area data is served entirely by `lib/constants/areas.ts` (the single source of truth). All components (AreaFilterPills, BrowseByArea, PartnerForm dropdown, generateMetadata) import from constants, not from database queries
...
- [ ] Install any missing shadcn components: `npx shadcn@latest add sheet` (for mobile nav slide-out), `npx shadcn@latest add button` (if not already installed)
...
- [ ] Create or update `app/(public)/layout.tsx`:
  - Wrap all public pages: `<Navbar />` + `<main>{children}</main>` + `<Footer />`
  - Cream background on main content area
- [ ] Update `app/(public)/page.tsx` with a temporary placeholder heading ("GoOut Hyd -- Coming Soon") to confirm layout renders correctly
...
- [ ] Create `app/(public)/cafes/page.tsx` as async Server Component:
...
- [ ] Create `components/cafes/AreaFilterPills.tsx` (`"use client"`):
...
- [ ] Create `components/cafes/CafeCard.tsx`:
...
- [ ] Build responsive card grid in listing page: `grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6`
- [ ] Create `components/cafes/CafeEmptyState.tsx`:
  - Message: "No cafes in [Area] yet -- check back soon!"
  - "Browse All Areas" button (shadcn Button, secondary variant) linking to `/cafes`
- [ ] Add `generateMetadata` function to cafes page:
...
- [ ] Create `app/(public)/cafes/[slug]/page.tsx` as async Server Component:
...
- [ ] Build cover photo hero section:
...
- [ ] Create `components/cafes/QuickContactBar.tsx`:
...
- [ ] Build About section: cafe description paragraphs on cream background, DM Sans body text
- [ ] Create `components/cafes/PhotoGallery.tsx`:
...
- [ ] Create `components/cafes/PhotoLightbox.tsx` (`"use client"`):
...
- [ ] Create `components/cafes/MenuHighlights.tsx`:
...
- [ ] Create `components/cafes/CafeUpcomingEvents.tsx`:
...
- [ ] Create `app/(public)/events/page.tsx` as async Server Component:
...
- [ ] Create `components/events/CategoryFilterCards.tsx` (`"use client"`):
...
- [ ] Build responsive event card grid: `grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6`
- [ ] Create `components/events/EventEmptyState.tsx`:
...
- [ ] Create `components/events/EventInfoCard.tsx`:
...
- [ ] Build event description section: full description on cream background, DM Sans body text
- [ ] Create `components/events/VenueSection.tsx`:
...
- [ ] Install shadcn components if not present: `npx shadcn@latest add select input label`
...
- [ ] Create `components/partner/PartnerForm.tsx` (`"use client"`):
...
- [ ] Add Resend package: run `npm install resend` in `apps/web/`
- [ ] Create `lib/email.ts`:
...
- [ ] Wire PartnerForm component to Server Action: import action, call on submit, handle response with toast
- [ ] Add `generateMetadata` to partner page: title "List Your Cafe | GoOut Hyd", description "Join Hyderabad's only platform built for independent cafes"
...
- [ ] Update `app/(public)/page.tsx` (replace temporary placeholder):
...
- [ ] Create `components/landing/BrowseByArea.tsx`:
...
- [ ] Create `components/landing/FeaturedCafes.tsx`:
  - Milk background section
...
- [ ] Create `components/landing/UpcomingEventsSection.tsx`:
  - Cream background section
...
- [ ] Create `components/landing/PartnerCTABanner.tsx`:
  - Espresso background, full-width container
...
- [ ] Add `generateMetadata` to landing page:
...
- [ ] Add `generateMetadata`: title "About | GoOut Hyd", description "The story behind Hyderabad's cafe discovery platform"
...
- [ ] Add `generateMetadata` to both pages: "Privacy Policy | GoOut Hyd", "Terms of Service | GoOut Hyd"
...
- [ ] Create `app/robots.ts`: export default robots config allowing all crawlers, pointing to sitemap URL
- [ ] Create `app/sitemap.ts`: generate dynamic sitemap with:
...
- [ ] Create `app/not-found.tsx`: custom 404 page with GoOut Hyd branding:
...
- [ ] Rewrite `SETUP.md` for GoOut Hyd:
...
[Goal: Replace sample data with real cafe information from Wilson and upload production images]

- [ ] Upload production images to Supabase Storage `images` bucket: real cafe cover photos, gallery images, event cover images
- [ ] **Do NOT run `npm run db:seed` against production** -- the seed script contains fictional data ("The Roast", "Cafe Blend") and will overwrite real records. Clear all seed data from the database before or after the first real cafe is added.
- [ ] Enter real cafe information directly via Supabase dashboard:
  - Add cafe records with real names, descriptions, addresses, phone numbers, Instagram handles, Google Maps URLs, opening hours
  - Add cafe_images with real Supabase Storage URLs for each cafe
  - Add menu_items with real menu highlights and prices
  - Add events with real upcoming events

### 7.3 Email & Analytics Configuration

[Goal: Enable production email notifications to Wilson and analytics tracking for page views]

- [ ] Create Resend production account at resend.com (or configure existing dev account for production)
- [ ] Add and verify sending domain in Resend dashboard
- [ ] Add `RESEND_API_KEY` to Vercel environment variables (production)
- [ ] Configure Wilson's notification email address in the application
- [ ] Enable Vercel Analytics in Vercel project dashboard (Settings → Analytics)
- [ ] Enable Vercel Speed Insights in project dashboard (optional, zero-config)

### 7.4 Production Deploy & Favicon

[Goal: Deploy the complete application to production with proper branding assets]

- [ ] Generate favicon set from GoOut Hyd wordmark using realfavicongenerator.net: favicon.ico, apple-touch-icon.png, site.webmanifest
- [ ] Add favicon files to `app/` directory and configure in root layout metadata
- [ ] Run `npm run lint` and `npm run type-check` in `apps/web/` to catch any issues before deploy
- [ ] Deploy to production via git push to main (Vercel auto-deploys)
- [ ] Configure all production environment variables in Vercel dashboard if not already set
