        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:gap-8">
            {valueCards.map(({ icon: Icon, title, body }) => (
              <div
                key={title}
                className="rounded-xl border border-brand-border bg-foam p-6 shadow-sm transition-shadow hover:shadow-md"
              >
                <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-caramel/15 text-caramel">
                  <Icon className="h-6 w-6" aria-hidden />
                </div>
                <h3 className="mt-4 font-heading text-xl text-espresso">{title}</h3>
                <p className="mt-2 text-base text-roast/80">{body}</p>
              </div>
            ))}
          </div>
        </div>
            ...
            {steps.map(({ icon: Icon, title }, index) => (
              ...
              >
                {index > 0 ? (
                  <>
                    <ChevronDown
                      className="mb-2 h-6 w-6 shrink-0 text-caramel/40 md:mb-0 md:hidden"
                      aria-hidden
                    />
                    <ChevronRight
                      className="mb-2 hidden h-6 w-6 shrink-0 text-caramel/40 md:mb-0 md:block"
                      aria-hidden
                    />
                  </>
                ) : null}
