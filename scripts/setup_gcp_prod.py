...
def grant_user_cloud_build_permissions(config: dict[str, Any]) -> None:
    ...
    try:
        ...
        # Grant Service Account User role to deploy Cloud Functions with custom service accounts
        # Need permissions on all service accounts for the split architecture
        env_suffix = (
        ...
        service_accounts_to_grant = [
            f"rag-processor-{env_suffix}@{config['project_id']}.iam.gserviceaccount.com",
            f"rag-gcs-handler-{env_suffix}@{config['project_id']}.iam.gserviceaccount.com",
            f"rag-task-processor-{env_suffix}@{config['project_id']}.iam.gserviceaccount.com",
        ]

        log("   🔑 Granting Service Account User permissions for deployment...")
        granted_count = 0

        for sa_email in service_accounts_to_grant:
            try:
                log(f"   Granting serviceAccountUser role on: {sa_email}")
                run_command(
                ...
                log(f"   ✅ Service Account User role granted for {sa_email}")
                granted_count += 1
            except Exception as e:
                error_str = str(e).lower()
                if "already has" in error_str or "already contains" in error_str:
                    log(
                        f"   ✅ Service Account User permission already exists for {sa_email}"
                    )
                    granted_count += 1
                else:
                    log_warning(
                        f"Could not grant Service Account User role for {sa_email}: {e}"
                    )
                    log("   💡 You may need to manually grant this role:")
                    log(
                        f"      gcloud iam service-accounts add-iam-policy-binding {sa_email} \\"
        ...
        if granted_count == len(service_accounts_to_grant):
            log_success(
                f"Service Account User roles granted to {user_email} on all service accounts"
            )
            log(
                "   ✅ User can now deploy Cloud Functions and Cloud Run services with custom service accounts"
            )
        else:
            log_success(
                f"Service Account User roles configured ({granted_count}/{len(service_accounts_to_grant)} successful)"
            )
            if granted_count < len(service_accounts_to_grant):
                log("   ⚠️  Some permissions may need manual configuration")
